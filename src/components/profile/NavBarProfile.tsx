...
const NavBarProfile: React.FC<NavBarProfileProps> = ({ className }) => {
      ...
      {welcomeData.map((data, index) => {
        ...
        return (
          <Link href={href} key={index} className="flex my-10">
            ...
            <div className={path.startsWith(href.toString()) ? "flex justify-center items-center ml-10 freelance-active" : "flex justify-center items-center ml-10" }>
              <h5 className="font-normal text-xl" style={path.startsWith(href.toString()) ? {fontWeight: "bold", borderBottom: `5px solid ${color}`, paddingBottom: "0.5rem"} : undefined}>{title}</h5>
            </div>
          </Link>
        );
      })}
