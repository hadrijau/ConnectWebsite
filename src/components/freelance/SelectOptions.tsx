...
const SelectOptions = () => {
  ...
  const removeCompetence = (option: string): void => {
    const updatedOptions = selectedCompetences.filter((selectedCompetence) => selectedCompetence !== option);
  ...
  return (
    <div className="flex-col mt-5 ml-20">
      ...
      {selectedCompetences.map((competence) => (
        ...
        >
          <span>
            <span className="mr-3 text-xl cursor-pointer" onClick={() => removeCompetence(competence)}>x</span>
            {competence}
          </span>
        </div>
      ))}
